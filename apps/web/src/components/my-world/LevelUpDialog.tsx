...
export function LevelUpDialog({ level, onClose }: LevelUpDialogProps) {
  ...
  const detail =
    level !== null && COSMETIC_LEVELS.has(level) ? t(`levelUpDetail.${level}`) : t("levelUpDetail.default");
    ...
    >
      {level !== null ? (
        <div className="flex flex-col items-center px-6 py-8 text-center">
          <div className="size-24 overflow-hidden rounded-full bg-brand-soft shadow-md ring-4 ring-brand-soft">
            <IllustratedAvatar level={level} />
          </div>
          <h2 id={titleId} className="mt-5 text-2xl font-bold text-content">
            {t("levelUpTitle")}
          </h2>
